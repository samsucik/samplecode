 ...
 */
		...
		$sort_par = esc_sql($_REQUEST["s"]);
		$uni = esc_sql($_REQUEST["u"]);
		$year = esc_sql($_REQUEST["y"]);
		$area = esc_sql($_REQUEST["a"]);
		$country = esc_sql($_REQUEST["c"]);
		...
		**********************************************************************/
		...
		if (count($loop_table) > 0) 
		{
			...
			foreach ($loop_table as $k => $loop_row) {
				...
				if ($sort_par != "0" and $loop_row->$sort_par != $oldHeading){
					...
					echo "<div class='alumni-category-separator'>" . htmlspecialchars($loop_row->$sort_par) . 
						"<a href='#content' class='to-top'>" . 
						htmlspecialchars($lang == 'sk' ? "Návrat hore" : "Back to top") . "</a></div>";
				}
		        ...
		        echo "<div class='alumni-box'><div class='alumni-uni'>" . 
		        	htmlspecialchars($sort_par == "uni" ? $loop_row->degree : $loop_row->uni) . 
		        	"</div><div class='alumni-subject'>" . htmlspecialchars($loop_row->degree) . "</div>" . 
		        	(is_null($loop_row->skype) ? "" : "<div style='display: none;width: 100%;height: auto;position: absolute;" . 
					" bottom: 35px;z-index: 1000;color: rgb(240,240,255);background-color: rgba(20,10,0,0.90);' id='skype-" . 
					htmlspecialchars($loop_row->id) . "'><b>Skype</b>: <i>" . htmlspecialchars($loop_row->skype) . "</i></div>") . 	
					"<img src='https://ib.gjh.sk/wp-content/uploads/alumni/" . htmlspecialchars($loop_row->photo) . 
					"' class='alumni-box-image'><div class='alumni-social-panel'><span class='alumni-name-span'>" .
					htmlspecialchars($loop_row->name) . "</span><span class='alumni-name-span surname'> " . 
					htmlspecialchars($loop_row->surname) . "</span><div style='clear:both;'> </div>";
					...
					if ($loop_row->$key != NULL && $social_counter < 4){
						...
						switch ($key) {
							...
							// treated differently; e-mail opens the "compose" window, skype shows the nick
						 	case 'email':
						 		echo "<a class='alumni-email' id='" . htmlspecialchars($loop_row->id) . "' name='" .
						 			htmlspecialchars($loop_row->name) . " " . htmlspecialchars($loop_row->surname) . 
						 			"' onclick='jQuery.fn.writeAlumniEmail(this);'";
						 		break;
						 	case 'skype':
						 		echo "<a onclick='jQuery.fn.showSkype(this);' id='" . htmlspecialchars($loop_row->id) . "' ";
						 		break;
						 	default:
						 		echo "<a href='" . htmlspecialchars($loop_row->$key) . "' ";
						 		break;
						}

						// tooltip in the correct language
						echo "title='" . ($lang == 'sk' ? "Spoj sa s absolventom " . htmlspecialchars($loop_row->name) . 
							" cez " : "Get in touch with " . htmlspecialchars($loop_row->name) . 
							" via ") . htmlspecialchars($value) . "' target='_blank'><img src='https://ib.gjh.sk/wp-content/uploads/alumni/" . 
							htmlspecialchars($key) . ".svg'></a>";
					} 							
				}
				echo "</div></div>";			
			}
		...
		// report if no results match the query or if something simply went wrong...
			...
			echo "<div class='alumni-category-separator'>" . ($lang == 'sk' ? "Žiadne výsledky, skúste zmeniť filtre." : 
